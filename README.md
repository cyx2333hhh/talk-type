...
Talk-type 面向 macOS 的中文和中英混合语音输入。它将实时语音识别、文本整理和跨应用输入整合在一起，适合写作、聊天、笔记和日常办公。
...
首次使用时，在「设置 → 权限」中开启麦克风、语音识别和辅助功能权限。AI 整理为可选功能，可在「设置 → 识别与整理」中选择服务、填写 API Key 和模型 ID；不同服务的配置独立保存。
...
Talk-type is a macOS voice input tool for Chinese and mixed Chinese-English input. It brings live speech recognition, text cleanup, and cross-app input together for writing, messaging, note-taking, and everyday work.
...
- **Mixed Chinese-English input**: Supports English names, product names, technical terms, and a custom vocabulary.
...
On first use, grant Microphone, Speech Recognition, and Accessibility permissions in **Settings → Permissions**. AI cleanup is optional. In **Settings → Recognition & Cleanup**, choose a provider and enter its API key and model ID; settings are stored separately for each provider.
