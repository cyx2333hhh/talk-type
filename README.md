...
- **Apple Speech 优先**：动态识别结果直接作为主文本；仅在没有结果时使用本地 Whisper 回退。
...
<p align="center"><sub>实时预览、上下文和文本整理均可单独设置。</sub></p>

### 隐私

- Apple Speech 是主识别；Whisper Small 仅在 Apple Speech 没有结果时提供本地回退。
- 临时录音会在本次处理结束后删除。
...
- **Apple Speech first**: The visible live transcript remains authoritative; local Whisper is only a fallback when Apple Speech returns no text.
- **Lightweight workflow**: Global shortcut, menu bar controls, live preview, recent history, and clipboard fallback.

### Privacy

Apple Speech is the primary recognizer. Local Whisper fallback does not upload audio. Optional AI cleanup receives text and optional cursor context only when enabled; audio is never sent with that request. API keys are stored in the macOS Keychain, and recent results remain on the Mac.
