...
struct DeepSeekClient {
    ...
    func correct(_ text: String,
        ...
        规则：
        1. 先理解完整句意，再添加必要标点并合理断句、分段；不要机械照搬语音停顿，也不要把一句完整的话切得过碎；
        ...
        7. 中文语境使用全角标点。并列词用顿号，分句用逗号，完整句子补句末标点；不要在固定英文词组、产品名、缩写、版本号、数字、小数、URL 或代码内部插入中文标点；
        ...
        15. 如果光标右侧同一行仍有已有正文，用户很可能是在句中补词或替换。短词、名称和短语不要自动添加句号、问号或感叹号，也不要重复右侧已有标点；只有原始语音明确表达完整独立句时才保留句末标点。
