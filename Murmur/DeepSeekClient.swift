...
struct AIClient {
    ...
    func correct(_ text: String,
                 ...
                 bilingualAssistTranscript: String? = nil,
                 primaryLocale: String = "zh-CN",
        ...
        let isEnglishPrimary = primaryLocale.lowercased().hasPrefix("en")
        let primaryLanguage = isEnglishPrimary ? "英文" : "中文"
        let assistLanguage = isEnglishPrimary ? "中文" : "英文"
        ...
        8. 标点要与原文语言一致：中文使用全角标点，英文使用英文半角标点；中英文混合时按每段实际语言处理。不要在固定英文词组、产品名、缩写、版本号、数字、小数、URL 或代码内部插入中文标点；
        9. 对中英混合内容要优先保留中文和英文专名、产品名、技术词和缩写的标准写法；可参考用户词库纠正相近音译或错误大小写，但不要凭空添加未表达的信息；
        10. 如果提供了备选转写，它来自同一段音频，只能用于发现原始转写中非常明确的识别错误。两份结果冲突且无法仅凭句意确定时，以原始转写为准，绝不能整句替换。
        11. 当前主识别语言是\(primaryLanguage)。如果提供了辅助转写，它来自同一段音频的\(assistLanguage)识别。主转写决定句子结构和主要内容，辅助转写只用于恢复或校正另一种语言的单词、短语、缩写和大小写，不要直接用它替换整句。
        ...
        18. 陈述句、祈使句和完整说明句补上与所在语言一致的句末标点；有真实疑问语气或疑问结构才使用问号。中文列举项之间用顿号，较长分句之间用逗号；英文按正常英文标点处理，需要引出解释或列表时使用冒号。
        ...
        let vocabulary = contextualStrings
            .prefix(80)
            .joined(separator: "\n")
        let bilingualAssist = bilingualAssistTranscript?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let bilingualAssistSection = bilingualAssist.flatMap { $0.isEmpty ? nil : $0 } ?? "无"
        ...
        同一段音频的\(assistLanguage)辅助转写（可能有误，只用于判断另一种语言的词）：
        <bilingual_assist_transcript>
        \(bilingualAssistSection)
        </bilingual_assist_transcript>
